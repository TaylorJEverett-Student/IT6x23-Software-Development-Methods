...
public class Unique2
{
   public static void main(String[] args)
   {
      ...
      int[] numbers = new int[30]; // array that has 5 integers
      int count = 0; // numbers given are stored for use
      
      while(count < numbers.length)//loop that starts at 0 and stops at 30
      {
         System.out.print("Enter number: ");//asks for a number
         int number = input.nextInt();//takes number into int number
         
         // checks the input to be between 10 and 500
         if (10 <= number && number <= 500)
         {
